...
<x-layouts.landingpage-layout >
    ...
    <div class="w-full" x-data="{accesorypdf: "",pdf_page: 0, accesorypdf_id: 0, services_description: "", filters: false  }">
        <div class="relative max-h-[20rem]  w-full overflow-hidden flex bg-black">
            <div class="w-full max-h-[50rem]  object-cover">
                <img src="{{ asset('img/slider1.jpg') }}" alt="services_img" class="w-full xs:h-full md:h-[60vh] object-cover" style="object-position: 0 10%;"/>
            </div>
            <div class="absolute w-full max-h-[50rem]  inset-0  block bg-opacity-40 bg-black">
            </div>
            <div class="absolute bottom-0 mx-auto w-full">
                <p class="text-5xl text-white font-semibold mb-24 mx-auto text-center uppercase">{{ $info }}</p>
            </div>
        </div>
        <section>
            <div class="w-full items-center justify-center pt-12 bg-white ">
        ...
        <section class="">
            ...
            <main class="relative bg-white">
                ...
                <div id="appFlipbook">
                    @foreach ($filteredContent as $index => $product)
                        {{-- Productos --}}

                        @if(!$product->is_catalog)
                            <div class="{{ $index  === 0 ? 'pb-28' : 'py-28' }} flex  {{ $index % 2 === 0 ? 'bg-white' : 'bg-slate-200' }}">
                                <div class="flex w-full justify-center xs:flex-col sm:flex-row gap-10  pr-6 xl:pr-0 ">
                                    ...
                                    <div class="md:sticky top-32 h-fit xs:mx-auto md:mx-0">
                        ...
                        @else 
                            <div class="{{ $index  === 0 ? 'pb-28' : 'py-28' }} xs:px-4 sm:px-14 md:px-28 py-1  w-full {{ $index % 2 === 0 ? 'bg-white' : 'bg-slate-200' }}">
                                <h2 class="mb-2 font-manrope font-bold text-4xl text-center  leading-10 text-gray-900 uppercase">
                                    {{ $product->name }}
                                </h2>
                                <h3 class="mb-2 font-manrope font-semibold text-2xl mt-2 text-center leading-10 text-sky-600">
                                    {{ $product->brand->name }}
                                </h3>
                                <flipbook-container 
                                    :src="{{ json_encode(asset('storage/'.$product->catalogpdf)) }}" 
                                    :index="{{ $index }}">
                                </flipbook-container>
                            </div>
                        @endif
...
<script>
    ...
    function showSuppliesModal(title, shortDesc, imgurl) {
        document.getElementById('suppleImage').src = "";
        // Actualizar el contenido del modal
        document.getElementById('offertModalTitle').textContent = title;
        console.log(imgurl)
        const fullImageUrl = '/storage/' + imgurl;
    ...
    function showManualModal(title, shortDesc, imgurl) {
        document.getElementById('suppleImage').src = "";
        // Actualizar el contenido del modal
        document.getElementById('offertModalTitle').textContent = title;
        console.log(imgurl)
...
document.querySelectorAll('[id^="carousel-"]').forEach(carousel => {
    ...
    // Cada vez que cambie de slide → pausar videos
    carousel.addEventListener('slide.tw.carousel', () => {
        console.log("cambio de slide")
        videos.forEach(v => {
            v.pause();
            v.currentTime = 0; // opcional: reinicia
        });
    });
});
